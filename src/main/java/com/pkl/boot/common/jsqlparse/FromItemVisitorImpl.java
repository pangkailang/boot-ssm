...
import com.pkl.boot.common.util.UserUtils;
...
public class FromItemVisitorImpl implements FromItemVisitor {
            ...
            if (test!=null) {
                ...
                for (TableCondition tableCondition : test) {
                    ...
                    if ("between".equalsIgnoreCase(tableCondition.getOperator())|| "not between".equalsIgnoreCase(tableCondition.getOperator())) {
                        Column column = new Column(new Table(tableName.getAlias()==null?tableName.getName():tableName.getAlias().getName()),"ids");
                       // expressions = new Expression[] {column,new StringValue("44444")};
                        expressions = new Expression[] { column,new LongValue("3333"),new LongValue(tableCondition.getFieldValue()) };
    ...
    public void visit(SubSelect subSelect) {
        ...
        Column column = new Column(new Table(subSelect.getAlias().getName()),"id");
        expressions = new Expression[] {column,new StringValue("44444")};
        Expression operator = this.getOperator(
                "=", expressions);
        if (this.enhancedCondition != null) {
